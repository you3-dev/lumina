...
### ✅ Phase 5: コンテンツ実装と最終調整 - 【完了】

- **Task 5.1: 海上・水中モンスターのAI実装**
  - エリア5用のモンスターデータ（セイレーン、クラーケン、リヴァイアサン等）を `data.js` に追加。ボス用の行動パターン（2回行動、スキル）はデータ定義にて実装済み。
- **Task 5.2: エリア5専用アイテム・報酬の定義**
  - 「人魚の鱗」「船の呼び笛」「トライデント」などを `data.js` に追加。
- **Task 5.3: 最終ボス「リヴァイアサン」戦の構築**
  - `battle.js` にボス撃破時のフラグ更新（シナリオ進行用）を追加。
