...
### Step 7: 最終確認 (Final QA) [COMPLETED]
- **ショップ・施設連携**: ポルティアの店や宿屋がNPC会話後に正しく開くよう実装しました。
- **セーブ・ロード対応**: 船の状態、酸素、歩数（ギガント移動用）がセーブデータに保存されることを確認。
- **入力制御の修正**: ショップや宿屋のメニューをBボタンで閉じられるように最小限の入力を実装。
- **フラグ整合性**: 3つの涙を集めてアトランティアの結界を解くメインフローが理論上完成しました。

### Step 8: 港町ポルティアの完成 (Completion of Port Portia)
仕様書通りの施設とNPCを充実させ、拠点の完成度を高めます。
- **施設拡張**: `town_portia.json` に武器屋・道具屋・教会・造船所の建物を視覚的に追加。
- **NPC追加**: 吟遊詩人（ヒント）、船の近くの子供、教会の神父などを配置。
- **隠しアイテム**: 樽や建物の裏にゴールドやステータスアップの種を配置。

### Step 9: ギミックとボスの詳細化 (Detailing Gimmicks and Bosses)
「設計書との乖離」があった特殊ギミックとボス戦を、本来の仕様に近づけます。
- **音のパズル**: 珊瑚の迷宮で、移動ごとに正解・不正解を知らせるSEを鳴らす処理を実装。
- **監獄島の巡回兵**: 決まったルートを往復し、接触すると逃走不能の強制戦闘が発生するNPCを配置。
- **ギガント体内の酸タイル**: 踏むと固定ダメージを受ける床トラップの実装。
- **ボスAIとスキルの追加**: リヴァイアサンの属性変化AI、セイレーンの全体眠り、バルバロイの物理特化AIを実装。

### Step 10: 最終クエストと導線の整備 (Final Quest and Navigation)
ストーリーのクライマックスを演出し、次エリアへの接続を確実にします。
- **中ボス「アルビダ」**: 出航直後の外海で進路を塞ぐイベントバトルを実装。
- **海神の祭壇**: 3つの涙を捧げるイベントシーンと演出を実装。
- **「天空の海図」の入手**: リヴァイアサン撃破後、アイテムを入手しエリア6への導きセリフを実装。
