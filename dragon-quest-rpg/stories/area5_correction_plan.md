...
### Step 4: メカニクスと高度なギミックの実装 (Mechanics & Gimmicks)
「ただ歩けるだけ」から「設計書通りのプレイ感」へ引き上げます。
- **船舶速度の動的変更**: `engine.js` の `partyData.moveProgress` 計算に `vehicle` 倍率（1.5倍〜2倍）を導入。
- **魔力封印エリアの実装**: `battle.js` で `currentMap.sealSpells` フラグがある場合、呪文コマンドを封印。
- **酸素回復ポイントの実装**: マップ上の特定タイル（泡 / TILE.OASIS等で代用）に触れた際の `oxygen` 全回復処理。
- **潮流ギミック**: 珊瑚の島周囲などで、特定方向へ強制移動させるロジックの追加検討。

### Step 5: マスターデータの完全同期 (Data Completion)
`area5_data_sheet.md` の内容を `data.js` へ完全に反映します。
- **全モンスター追加**: ウミスライム、しびれクラゲ、アイアンタートル等、属性耐性とスキルを含めて定義。
- **全装備・アイテム追加**: ポセイドンブレード、酸素缶、海の香草等の追加とショップ(Portia)への陳列。
- **遭遇テーブルの精緻化**: 海域ごとの難易度調整を反映。

### Step 6: 「海神の涙」収集とアトランティア攻略 (Quest & Flags)
エリア5のメインストーリーラインをロジカルに完結させます。
- **三至宝フラグとアイテムの実装**: `state.js` に `tearOfBlue`, `tearOfRed`, `tearOfGreen` を追加、各ボスのドロップとして設定。
- **結界解除イベント**: アトランティア入口で3つの涙の所持を確認する判定と扉開放ロジック。
- **移動島「ギガント」の簡易実装**: 時間経過または歩数によって外海上の特定座標に出現・移動するスクリプト。
- **リヴァイアサン決戦の強化**: 2回行動、特定HPでの属性変化や「大海嘯（全体攻撃）」のAI強化。
